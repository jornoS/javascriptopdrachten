...
.then(function(myJson) {
  ...
  for(i = 0; i < myJson.results.length; i++){
    personen[i] = {firstname: personen[i].name.first, 
    lastname: personen[i].name.last,
    image: personen[i].picture.thumbnail, 
  ...
  console.log(personen);
    ...
    for (i = 0; i < personen.length; i++){
     ...
     window.onclick = function(event) {
      ...
      info.innerHTML='<i class="fa fa-envelope-o"></i>  Email: ' + personen[i].email + '<br><br>' + 
      '<i class="fa fa-phone"></i>  Phonenumber: ' + personen[i].phone + ' <i class="fa fa-commenting"></i><br><br>' + 
      '<i class="fa fa-home"></i> <br>' +
      'City: ' + personen[i].city + '<br>' +
      'State: ' + personen[i].state + '<br>' +
      'adress: ' + personen[i].street;
